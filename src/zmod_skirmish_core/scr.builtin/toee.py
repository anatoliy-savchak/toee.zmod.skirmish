class PyObjHandle(object):
	...
	def npc_waypoints_get(self):
		"""npc.npc_waypoints_get() -> list()"""
		return list()

	def npc_waypoints_set(self, list_of_waypoints, loop_reset = 0):
		"""npc.npc_waypoints_set(list_of_waypoints, loop_reset = 0) -> None"""
		return
	...
	def spells_cast_reset(self):
		return
	
	def spells_memorized_forget(self, classEnum):
		# if classEnum == -1 then all
		return
...
class game(object):
	...
	@staticmethod
	def get_spell_mesline(line_id):
		""" game.get_spell_mesline(line_id: int) -> str"""
		return "str"

	@staticmethod
	def get_mesline(file_path_rel, line_id):
...
def any(objects, method_name, method_args):
	"""any(objects: list, method_name: string, method_args: object) -> PyObjHandle
	returns first obj which method's returned true
	"""
	return PyObjHandle(0)
...
# D20 Attack Power
D20DAP_NORMAL = 1 # for bypass it means always
