...
def consider_target(attacker, target, aiSearchingTgt=False):
	do_print = game.combat_is_active()
	if do_print: debug_print("consider_target attacker: {}, target: {}, aiSearchingTgt: {}".format(attacker, target, aiSearchingTgt))
	...
	if target_flags & ignore_flags: 
		if do_print: debug_print("consider_target:ignore_flags")
		return 0
	...
	if not target.is_critter():
		if isBusted(target): 
			if do_print: debug_print("consider_target: is busted")
			return 0
	else:
		if aiListFind(attacker, target, AI_LIST_ALLY): 	
			if do_print: debug_print("consider_target:ai list ally")
			return 0
		...
		if t_leader != OBJ_HANDLE_NULL and t_leader == a_leader:
			if do_print: debug_print("consider_target:same leader")
			return 0
		...
		if target.is_friendly(attacker): 
			if do_print: debug_print("consider tgt: is friendly: " + str(target) + "\n")
			return 0

		if target.distance_to(attacker) > 125.0: 
			if do_print: debug_print("consider_target:too far")
			return 0

	if a_leader != OBJ_HANDLE_NULL:
		if a_leader.distance_to(target) > 50:
			if attacker.distance_to(target) > 50.0:
				if do_print: debug_print("consider tgt: too far from leader")
				return 0

	if do_print: debug_print("consider tgt: good one")
	return 1

def getFriendsCombatFocus(obj, friend, leader, aiSearchingTgt = False):
	debug_print("getFriendsCombatFocus obj: {}, friend: {}, leader: {}, aiSearchingTgt: {}".format(obj, friend, leader, aiSearchingTgt))
